...
def viterbi_log(states, emission, transition, sentence): #emission and transition are already logged
    e = emission
    t = transition
    n = len(sentence)
...
def perceptropUpdate(gold, predicted, sentence_words, e, q):
...
def test_parameters(iteration):
    ...
    final = []
    for sen in biglist:
        scores, path = viterbi_log(lab,e_avg,q_avg,sen)

        for i in path:
            if i != '***START***':
                final.append(i)

    f = open("datasets/EN/test_results/dev.p5-avg-" + str(iteration) + ".out" ,"w+", encoding="utf8")
...
r1 =  open("datasets/EN/train","r",encoding='utf-8')
lines1 = r1.readlines()
r1.close()

#get lines of test data
r2 =  open("datasets/EN/dev.in","r",encoding='utf-8')
lines2 = r2.readlines()
r2.close()

#smooth the data
train, test = smoothEmission(5, lines1, lines2)
...
e = emission(train)
labels, q = transition("datasets/EN/train")
...
### training ##
train_file = open('datasets/EN/train', 'r')
Ygold = ['***START***']
...
for i in range(iterations):
    for sentence_pair in zip(smoothed_sentences, all_Ygold): 
        ...
        scores, path = viterbi_log(lab,e,q,sentence_pair[0]) #Predicted path
        ...
        if predicted_Y != goldtruth_Y:
            perceptropUpdate(goldtruth_Y, predicted_Y, smoothed_sentence_words, e, q)
            sum_param(e, q)
